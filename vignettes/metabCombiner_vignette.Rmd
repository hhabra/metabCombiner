---
title: Combining LC-MS Metabolomics Datasets with metabCombiner
author: "Hani Habra"
...
In the computational analysis of LC-MS metabolomics data, a key step is to align the measurements of identical compounds commonly represented as <mass-to-charge ratio (m/z), retention time (RT)> features. Feature alignment between datasets acquired under non-identical conditions presents many underexplored opportunities in untargeted metabolomics. The key challenge to address is achieving a correspondence between identical features, given that most are not easily identified . `metabCombiner` provides a list of all possible feature pair alignments (FPAs) and determines their validity through a pairwise similarity score. We will show here how to use `metabCombiner` to perform this cross-dataset alignment analysis.

#Input Requirements

The inputs for this workflow are two peak-picked and aligned LC-MS metabolomics data frames, with columns for m/z, RT, and individual sample abundance values. It is recommended, but not required, to have columns for feature identity and adduct type. As we will discuss, `metabCombiner` is flexible on what these columns may be named. Other input dataset columns are included in the final output, though these will not have any role in any of the outlined steps.

Typically you will need to load a dataset from file into an R data.frame, e.g. using this template. Be sure to set stringsAsFactors to FALSE.
...
There are three major assumptions that these datasets must conform to:

1) They have been acquired in the ionization mode (positive or negative)
2) The samples in both datasets are biologically similar: they possess a strong overlap in terms of their metabolomic composition
3) The elution order of metabolites is mostly preserved. Datasets acquired from excessively disparate chromatographic techniques (such as between RPLC and HILIC methods) are not suitable for this workflow.
...
We demonstrate each step on a pair of packaged human plasma metabolomics
datasets contained within the package. 
...
A `metabData` object is a specifically-formatted metabolomics dataset object used to stage`metabCombiner` analyses. The constructor function `metabData()` consists of two parts: a) detecting the specific sample columns and b) filtering features.
...
6) extra (optional): non-analyzed dataset columns
7) samples (required): sample abundance values
...
```{r, eval = FALSE}
p20 = metabData(plasma20, mz = "mz", rt = "rt", id = "id", adduct = "adduct", samples = "CHEAR.20min", ...)
...
p20 = metabData(plasma20, ..., id = c("compound|identity|name"),...)  #any one of these three keywords

##all of the following values for samples argument give the same result
p20 = metabData(plasma20, ..., samples = c("CHEAR.20min.1, CHEAR.20min.2, CHEAR.20min.3, CHEAR.20min.4, CHEAR.20min.5"), ...)     #explicit naming

p20 = metabData(plasma20, ..., samples = names(plasma20)[6:10], ...) #column name indexing

#recommended method: use a keyword common and exclusive to sample names of interest
p20 = metabData(plasma20, ..., samples = samples = "CHEAR", ...)  
p20 = metabData(plasma20, ..., samples = samples = "CH", ...) 
```
...
```{r, eval = FALSE}
p20 = metabData(plasma20, mz = "mz", rt = "rt", id = "id", adduct = "adduct", samples = "CHEAR", extra = c("Red", "POOL", "Blank"),...)

getSamples(p20)   #should return column names containing "CHEAR"
getExtra(p20)   #should return column names containing "Red Cross", "POOL", and "Blank"
```
...
The retention time range filter restricts features to those between the *rtmin* and *rtmax* arguments. By default these are set to the minimum and maximum observed retention times, but they should roughly correspond to the first and last observed shared metabolites. Consider the head and tail RTs of the *plasma20* dataset below. While the head appear to be at a normal start time of 0.5 min, the tail features however are spaced far apart, which may distort the downstream modeling steps. Therefore we set the *rtmax* argument to 17 min for this exercise, filtering five tailing features. 
...
```{r}
p20 <- metabData(table = plasma20, mz = "mz", rt = "rt", id = "identity", adduct = "adduct", samples = "CHEAR", extra = c("Red", "POOL", "Blank"), rtmin = "min", rtmax = 17, misspc = 50, measure = "median", zero = FALSE, duplicate = c(0.0025, 0.05))
```
...
Printing a metabData object gives a useful dataset summary and using `getStats` gives information on filtered features.

# Feature m/z Grouping and Pairwise Alignment Detection

With our two *metabData* objects, we proceed with our dataset combining. First we group features from the datasets by m/z and construct a *metabCombiner* object, the main construct for this program. This is done using the `metabCombiner` constructor function.
...
#Anchor Selection and RT Mapping Spline
...
p.combined.2 = selectAnchors(p.combined, windx = 0.03, windy = 0.02, tolmz = 0.003, tolQ = 0.3, useID = FALSE)
anchors = getAnchors(p.combined.2)
plot(anchors$rtx, anchors$rty, main = "Anchors Plot", xlab = "rtx", ylab = "rty")
```

Shown above is a rough outline of the path through which we may fit a nonlinear curve. The arguments *windx* and *windy* are retention time windows drawn in the X and Y direction around the anchor points; in general, smaller values for these window arguments increases the number of anchors (including outliers). *tolmz* and *tolQ* restrict the m/z and Q differences of selected anchors. *useID* modifies the anchor selection algorithm by first searching for shared identities (i.e. feature pairs where idx is the same as idy, case-insensitive) before searching for the remaining abundant feature pairs. *useID* is set to FALSE by default, but prior or acquired knowledge of matching features may be useful for enhancing this process.
...
```{r,fig.width= 5, fig.height=4, fig.align='center'}
plot(p.combined.3, main = "Example metabCombiner Plot", xlab = "P30 RTs", ylab = "P20 RTs", lcol = "blue", pcol = "black", lwd = 3, pch = 19, fit = "gam")
...
combined.table.2 = labelRows(combined.table, minScore = 0.5, maxRankX = 3, maxRankY = 3, method = "score", conflict = 0.2, remove = FALSE, balanced = TRUE)

##version 2: mzrt-based conflict detection
combined.table.3 = labelRows(combined.table, minScore = 0.5, maxRankX = 3, maxRankY = 3, method = "mzrt", conflict = c(0.003,0.5,0.003,0.2), remove = FALSE, balanced = TRUE)
```

Some agruments that must be specified are the *minScore*, *maxRankX* & *maxRankY* threshold values, as well as the *conflict* value. Conflicts occur between pairs of FPAs that share one feature in common and may require inspection to discern the correct match. There are two methods for detecting conflicts: 1) "score" and 2) "mzrt". In both, the top-scoring FPAs (rankX = 1 & rankY = 1) are used as a benchmark; if the difference in scores of the conflicting FPAs is small (first method), or the unshared feature is within a set m/z & rt tolerance (second method), then both FPAs are flagged. Otherwise, the lower-ranked FPA is deemed removable.
...
```{r, eval = FALSE}
write2file(p.combined.table, file = "plasma.Combined-Table.Report.txt", sep = "\t")
```
